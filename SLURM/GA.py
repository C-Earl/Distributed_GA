...
def write_gene(gene: dict, name: str, run_name: str):
  while not lock_gene(name, run_name):  # Attempt to acquire lock
    time.sleep(1)
  ...
  unlock_gene(name, run_name)           # Release lock after writing
...
def load_gene(name: str, run_name: str):
  ...
  unlock_gene(name, run_name)           # Release lock after reading
  return gene
...
def delete_gene(name: str, run_name: str):
  ...
  gene_path = file_path(pool_path, name) + ".pkl"
  os.remove(gene_path)
  unlock_gene(name, run_name)           # Release lock after deleting
...
class Server():
  def __init__(self, run_name: str, model_name: str, num_clients: int, recall: bool = False, **kwargs):
    ...
    if recall:
      ...
      if count > 10:
        test_marker("ENDED", "MADE IT TO END")
        return
